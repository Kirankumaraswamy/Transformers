...
class Transformer(nn.Module):
    ...
    def __init__(self, model_dimension, src_vocab_size, trg_vocab_size, number_of_heads, number_of_layers, dropout_probability, log_attention_weights=False):
        ...
        # later all of these will get deep-copied multiple times internally
    ...
    def decode(self, trg_token_ids_batch, src_representations_batch, trg_mask, src_mask):
        ...
        trg_representations_batch = self.decoder(trg_embeddings_batch, src_representations_batch, trg_mask, src_mask)  # Shape (B, T, D)

        # After this line we'll have a shape , decoder generator does a simple
        # linear projection followed by log softmax
        trg_log_probs = self.decoder_generator(trg_representations_batch)  # (B, T, V), where V - target vocab size

        trg_log_probs = trg_log_probs.reshape(-1, trg_log_probs.shape[-1])  # Reshape into (B*T, V)

        return trg_log_probs  # PyTorch's nn.KLDivLoss expects log probabilities
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, trg_representations_batch, src_representations_batch, trg_mask, src_mask):
        srb = src_representations_batch  # alias
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask):
        ...
        query, key, value = [net(x).view(batch_size, -1, self.number_of_heads, self.head_dimension).transpose(1, 2) for net, x in zip(self.qkv_nets, (query, key, value))]
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, embeddings_batch):
        assert embeddings_batch.ndim == 3 and embeddings_batch.shape[-1] == self.positional_encodings_table.shape[1], f'Expected (batch size, max token sequence length, model dimension) got {embeddings_batch.shape}'
